...
class Class_model extends CI_Model
{
    ...
    {
        $this->db->select('c.*, COUNT(s.studentId) AS total_students');
        $this->db->from('tbl_classes c');
        $this->db->join(
            'tbl_students s',
            's.classId = c.classId AND s.isDeleted = 0', 
            'left'
        );
        
        // This is the missing piece: filter the CLASSES by station
        $this->db->where('c.stationId', (int)$StationId); 
