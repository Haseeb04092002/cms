...
class Tasks extends MY_Controller
{
	...
	{
		$this->output->set_header('X-Page-Title: Task Assignment');
	...
	public function upload_task($case = 'student', $userId = '')
	{
		switch ($case) {
			case 'student':
				...
				$tasks = $this->db->get('tbl_tasks')->result();

				// Attach documents for each task
				foreach ($tasks as $task) {

					$this->db->where('referenceType', 'Student Task');
					$this->db->where('referenceId', $task->taskId);
					$this->db->where('isDeleted', 0);

					$task->documents = $this->db->get('tbl_documents')->result();
				}
	...
	public function student_task_pdf($taskId = 0)
	{
    ...
    ');
		...
		$html = $this->load->view(
			'pages/task/template_student_task_pdf',
			$data,
			true
		);
		...
		$dompdf->stream(
			'student_task_report_' . $taskId . '.pdf',
	...
	public function all_tasks($studentId = '')
	{
		$UserId = '';
		$StationId = '';
		$UserId = $this->session->userdata('user_id') ?? '';
		...
		switch ($UserRole) {
			case 'Admin':
				...
				if (isset($studentId) && !empty($studentId)) {
					$this->db->where('t.studentId', $studentId);
				}
				$this->db->where('t.stationId', $StationId);
				$this->db->where('t.isDeleted', 0);
			...
			case 'Teacher':
				...
				$this->db->join(
					'tbl_classes c',
					'c.classId = t.classId AND c.stationId = t.stationId AND c.isDeleted = 0',
					'left'
				);
				...
				$this->db->join(
					'tbl_students s',
					's.studentId = t.studentId AND s.stationId = t.stationId AND s.isDeleted = 0',
					'left'
				);

				/* Only classes assigned as headClassId for this teacher */
				$this->db->join(
					'tbl_class_subject_assignment a',
					'a.headClassId = t.classId 
					AND a.stationId = t.stationId
					AND a.teacherId = ' . (int)$UserId . '
					AND a.isDeleted = 0',
					'inner'
				);
				if (isset($studentId) && !empty($studentId)) {
