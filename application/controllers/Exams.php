...
class Exams extends MY_Controller
{
	public function exam_dashboard()
	{
		...
		// print_r($data);
		// die();
		$this->output->set_header('X-Page-Title: Exam Management');
		$this->load->view('pages/exam/exam_dashboard', $data);
	}

	public function create()
	{
		$this->output->set_header('X-Page-Title: Create Exam');
		$this->load->view('pages/exam/exam_create');
	}

	public function create_exam_for_student()
	{
		$this->output->set_header('X-Page-Title: Create Exam For Student');
	...
	public function create_exam_for_class($classid = '', $className = '', $sectionName = '')
	{
		...
		// print_r($data);
		// die();
		$this->output->set_header('X-Page-Title: Create Exam For Class');
	...
	public function select_class()
	{
		...
		$this->output->set_header('X-Page-Title: Select Class');
	...
	public function class_exams()
	{
		...
		$data['exams']   = $exams;
		$data['classId'] = $classId;
		$this->output->set_header('X-Page-Title: All Exams');
	...
	public function view_exam($examId = '')
	{
		...
		$this->output->set_header('X-Page-Title: View Exams');
	...
	public function save_exam($case = 'add')
	{
		...
		$this->output->set_header('X-Page-Title: Save Exam');
		exit(json_encode($Response));
	}
}
