...
class Cms extends MY_Controller
{
	...
	{
		$this->output->set_header('X-Page-Title: Dashboard');
	...
	public function save_subject_class_assign()
	{
		...
		$this->output->set_header('X-Page-Title: Class & Subject Assignment');
		exit(json_encode($Response));
	}
	...
	public function dashboard($case = 'Admin')
	{
		switch ($case) {
			case 'Admin':
				...
				$data['fees_paid'] = $db->select_sum('amountPaid')
					->where('isDeleted', 0)
				...
				$this->output->set_header('X-Page-Title: Admin Dashboard');
	...
	{
		$this->output->set_header('X-Page-Title: User Management');
		$this->load->view('pages/admin/users');
	}

	public function courses()
	{
		$this->output->set_header('X-Page-Title: All Courses');
